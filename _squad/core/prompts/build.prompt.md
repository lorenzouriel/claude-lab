...
- `_squad/_memory/preferences.md` — user preferences
- Best-practices files referenced by design.yaml agents (load on demand from `_squad/core/best-practices/`)
...
For agent personas, consult the relevant best-practices files from `_squad/core/best-practices/` that were loaded. Use the discipline knowledge (principles, techniques, quality criteria, examples) to create high-quality agents tailored to this specific squad.
...
format: {format-id}   # OPTIONAL — e.g., "instagram-feed". Pipeline Runner auto-injects from _squad/core/best-practices/
...
- [ ] Best-practices files referenced by `format:` fields in steps exist in `_squad/core/best-practices/`
