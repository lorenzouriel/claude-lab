...
> "Your browser sessions are saved in `_squad/_browser_profile/`. To clear a platform's session, just delete the JSON file (e.g., `instagram.json`). I'll ask before saving any new session."
...
Browser sessions are stored as JSON files in `_squad/_browser_profile/`:

- Instagram: `_squad/_browser_profile/instagram.json`
- YouTube: `_squad/_browser_profile/youtube.json`
- Twitter/X: `_squad/_browser_profile/twitter.json`
- LinkedIn: `_squad/_browser_profile/linkedin.json`

**Loading a session (Playwright CLI):**
```bash
npx playwright open --load-storage=_squad/_browser_profile/{platform}.json {url}
```

**Saving a session (Playwright CLI):**
```bash
npx playwright open --save-storage=_squad/_browser_profile/{platform}.json {url}
```
...
Session files are stored in `_squad/_browser_profile/` as JSON files (one per platform). This directory is gitignored and persists between runs.
...
6. **Step 2 — Save the session (quick — user is already logged in):**
   ...
   If yes:
   ```bash
   npx playwright open --save-storage=_squad/_browser_profile/{platform}.json {platform-url}
   ```
...
7. If the user declines saving, inform them:
   "No problem. Remember your cookies are still in `_squad/_browser_profile/` — delete the folder to clear everything."
...
Users can clear all saved browser sessions at any time by deleting the `_squad/_browser_profile/` directory or individual JSON files. After deletion, the next investigation will require a fresh manual login.
