...
- `_squad/_memory/company.md` — Company context for personalization
- `_squad/_memory/preferences.md` — User preferences (especially Output Language)
- `_squad/core/best-practices/_catalog.yaml` — Best-practices catalog
...
Read `_squad/core/best-practices/_catalog.yaml` to discover available best-practices files.
...
3. Read the full content of each selected best-practice file from `_squad/core/best-practices/{file}`
...
5. For each accepted skill:
   a. Read the skills engine from `_squad/core/skills.engine.md`
...
Read the user's preferred language from `_squad/_memory/preferences.md` → **Output Language**.
...
- Use the format system: assign `format: {format-id}` to each creator step (e.g., `format: instagram-feed`). The Pipeline Runner injects the format file from `_squad/core/best-practices/` automatically — do NOT manually embed platform knowledge in task files or agent definitions.
