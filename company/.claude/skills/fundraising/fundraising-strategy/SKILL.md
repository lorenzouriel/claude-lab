...
1. **Verification Checklist** (MUST READ FIRST):
   ```
   .claude/skills/fundraising/html-templates/VERIFICATION-CHECKLIST.md
   ```

2. **Base Template** (shared structure):
   ```
   .claude/skills/fundraising/html-templates/base-template.html
   ```

3. **Skill-Specific Template** (content sections & charts):
   ```
   .claude/skills/fundraising/html-templates/fundraising-strategy-planner.html
   ```
