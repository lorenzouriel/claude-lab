...
1. **Verification Checklist** (MUST READ FIRST):
   ```
   .claude/skills/metrics/html-templates/VERIFICATION-CHECKLIST.md
   ```

2. **Base Template** (shared structure):
   ```
   .claude/skills/metrics/html-templates/base-template.html
   ```

3. **Skill-Specific Template** (content sections & charts):
   ```
   .claude/skills/metrics/html-templates/customer-feedback-framework.html
   ```
